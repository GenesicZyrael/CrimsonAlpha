...
function s.filter1(c,e,tp,ft)
	if c:IsType(TYPE_SYNCHRO) and (ft>0 or (c:GetSequence()<5 and c:IsControler(tp))) then
		if c:IsCode(29609196) then
			return Duel.IsExistingMatchingCard(s.filter3,tp,LOCATION_DECK,0,1,nil,e,tp)
		else
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if code==29609196 then
		tc=Duel.SelectMatchingCard(tp,s.filter3,tp,LOCATION_DECK,0,1,1,nil,e,tp):GetFirst()
	else
		tc=Duel.SelectMatchingCard(tp,s.filter2,tp,LOCATION_DECK,0,1,1,nil,code,e,tp):GetFirst()
	end
