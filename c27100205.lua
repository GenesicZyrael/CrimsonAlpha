...
function s.spfilter(c,e,tp)
	return c:IsSetCard({SET_LSWARM}) and c:IsLevelBelow(4) 
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if e:GetLabel()==1 and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
		...
		if Duel.SendtoGrave(tc,REASON_COST+REASON_DISCARD) and aux.SelectUnselectGroup(g,e,tp,2,2,s.spcheck,0) then
